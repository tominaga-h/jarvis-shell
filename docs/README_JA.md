...
- **登録**: `complete -c CMD [-s X]... [-l LONG]... [-a 'WORDS'] [-d DESC] [-n COND]` で `CMD` に 1 個の補完仕様を追加します。`-c/--command` は必須です。`-s` は単一文字のショートフラグ（例: `-s v` で `-v`）、`-l/--long-option` はロングフラグ名（例: `-l verbose` で `--verbose`）を指定し、いずれも複数回指定して 1 回の呼び出しで複数フラグを登録したり、同じコマンドに対して `complete` を繰り返し呼んで蓄積させたりできます。`-a/--arguments` は空白区切り（クォート可）の静的な候補語リストを指定します。`-d/--description` は補完メニューに表示される説明文を設定します。`-n/--condition` は条件式を保存します（現時点では評価されません — 下記の注記を参照）。
