...
- **Warm daemon (`external_zsh_daemon`)**: A one-shot zsh invocation (spawn a fresh `zsh`, run `compinit`, complete, exit) typically costs 700-1100ms per Tab press — mostly process/PTY startup, not the completion itself. When `external_zsh_daemon = true` (the default), Jarvish instead spawns a single `zsh -i` **as a plain child process of Jarvish** the first time you use the zsh bridge in a session, and keeps reusing it for every subsequent Tab press. This is not a system service and involves no `launchd`/`launchctl` — it is a per-session child process that lives only as long as your Jarvish shell does, and is killed automatically when Jarvish exits (or when you turn the flag off). After the first (cold) request, warm requests only pay for the completion computation itself — typically tens of milliseconds. Editing the bridge zshrc (see below) is detected automatically (by its file modification time) and transparently restarts the daemon on the next Tab, so you never need to restart Jarvish after tweaking `fpath`/`compdef` entries. If a completion hangs or the daemon otherwise fails, that one Tab press returns no candidates and the daemon is torn down; the very next Tab press lazily spawns a fresh one. Set `external_zsh_daemon = false` to always use the one-shot invocation instead (this also serves as a manual escape hatch when troubleshooting the bridge); hot-reloadable via `source` — flipping it off immediately shuts down any running daemon, flipping it on spawns a new one lazily on the next zsh completion request.
